...
class DataManager:
    def __init__(self, sample_size=5, max_sample=40):
        self.sample_size = sample_size
        self.make_sample = max_sample
        try:
            with open("data_for_train.p", "rb") as f:
                self.data_set = pickle.load(f)
        except FileNotFoundError:
            with open("data_for_train.p", "wb") as f:
                self.data_set = [[[0], -1]]
        ...
        if self.latest_number_quantity >= sample_size:
